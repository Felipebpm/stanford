...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder('uint8', [None, img_height, img_width, nchannels * self.config.state_history], name='s')
        self.a = tf.placeholder('int32', [None], name='a')
        self.r = tf.placeholder('float32', [None], name='r')
        self.sp = tf.placeholder('uint8', [None, img_height, img_width, nchannels * self.config.state_history], name='sp')
        self.done_mask = tf.placeholder('bool', [None], name='done_mask')
        self.lr = tf.placeholder('float32', None, name='lr')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope):
            flattened_state = tf.layers.flatten(state)
            out = tf.layers.dense(flattened_state, units=num_actions, activation=None, reuse=reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        variable_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = q_scope)
        target_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = target_q_scope)
        target_op = [tf.assign(target_q[i], variable_q[i]) for i in range(len(variable_q))]
        self.update_target_op = tf.group(*target_op)
