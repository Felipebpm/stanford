...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
        self.action_logits = build_mlp(
            self.observation_placeholder, self.action_dim, scope,
            self.config.n_layers, self.config.layer_size,
            self.config.activation
        )
        self.sampled_action = tf.squeeze(tf.multinomial(self.action_logits, num_samples = 1), axis = 1)
        ...
        self.sampled_action = self.action_means + tf.exp(log_std) * tf.random_normal(tf.shape(self.action_means))
        self.dist = tf.contrib.distributions.MultivariateNormalDiag(self.action_means, tf.exp(log_std))
        self.logprob = self.dist.log_prob(self.action_placeholder)
