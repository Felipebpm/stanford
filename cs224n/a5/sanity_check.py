...
def question_1f_sanity_check():
    ...
    emded_size = 2
    dropout_rate = 0.
    highway_module = Highway(emded_size, dropout_rate)
    highway_module.linear_proj.weight.data = torch.Tensor([[0.5, 0.7], [0.3, 0.4]])
    highway_module.linear_proj.bias.data = torch.Tensor([0.1, 0.2])
    highway_module.linear_gate.weight.data = torch.Tensor([[0.1, 0.2], [0.3, 0.4]])
    highway_module.linear_gate.bias.data = torch.Tensor([0.1, 0.8])
    input_tensor = torch.Tensor([[0.5, 0.5], [3., 4.], [5., 6.]])
    output = highway_module.forward(input_tensor)
    expected_output = np.array([[0.6124, 0.5380], [4.0759, 2.7462], [6.5447, 4.1171]])
    output_expected_size = [batch_size, emded_size]
    assert(list(output.size()) == output_expected_size)
    print(output)
    assert(np.allclose(output.detach().numpy(), expected_output))
    print("Sanity Check Passed for Question 1f")
    print("-"*80)
