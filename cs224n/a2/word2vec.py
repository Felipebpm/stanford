...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    #  centerWordVector = centerWordVec.reshape(centerWordVec.shape[0], 1)
    ...
    loss = - np.log(y_hat_o)
    gradCenterVec = np.dot(outsideVectors.T, y_hat) - trueOutsideVec
...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = - np.dot(outsideVectors[outsideWordIdx], 1 - sigmoid(y_hat[outsideWordIdx]))
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] -= np.dot(centerWordVec.T, 1 - sigmoid(y_hat[outsideWordIdx]))
    for idx in indices[1:]:
        loss -= np.log(sigmoid(-y_hat[idx]))
        gradCenterVec += np.dot(outsideVectors[idx], 1 - sigmoid(-y_hat[idx]))
        gradOutsideVecs[idx] += np.dot(centerWordVec.T, 1 - sigmoid(-y_hat[idx]))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss += currLoss
        gradCenterVecs[centerWordIdx] += currGradCenterVec
