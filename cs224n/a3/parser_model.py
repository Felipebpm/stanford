...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        batch_size = w.size(0)
        index = w.view(-1, 1).expand(-1, self.embed_size)
        batch_embeddings = torch.gather(self.embeddings, 0, index).view(batch_size, self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.matmul(x, self.embed_to_hidden_weight).add(self.embed_to_hidden_bias))
        h_drop = self.dropout(h)
        logits = torch.matmul(h_drop, self.hidden_to_logits_weight).add(self.hidden_to_logits_bias)
