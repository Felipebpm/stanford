...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        transition_handlers[transition]()
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        mini_batch = unfinished_parses[0 : batch_size] if batch_size < len(unfinished_parses) else unfinished_parses
        ...
        for partial_parse, transition in zip(mini_batch, transitions):
            ...
            if not partial_parse.buffer and len(partial_parse.stack) == 1:
                # might pose problem with same sentences
                complete_parses.append(mini_batch.index(partial_parse))
        for i in reversed(complete_parses):
