...
def softmax(x):
    ...
    denominator = np.sum(exponentials, axis = 1, keepdims = True)#.reshape(x.shape[0], 1)
...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    parameters = {}
    parameters['W1'] = np.random.randn(input_size, num_hidden)
    parameters['b1'] = np.zeros((1, num_hidden))
    parameters['W2'] = np.random.randn(num_hidden, num_output)
    parameters['b2'] = np.zeros((1, num_output))
    return parameters
    # *** END CODE HERE  ***
...
def forward_prop(data, labels, params):
    ...
    cost = - np.mean(np.sum(np.multiply(labels, np.log(A2)), axis = 1))
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    # foward propagation
    A1, A2, cost = forward_prop_func(data, labels, params)
    ...
    dZ2 = A2 - labels # (m, K)
    gradients['W2'] = np.matmul(A1.T, dZ2) / m # (num_hidden, K)
    gradients['b2'] = np.sum(dZ2, axis = 0) / m # (1, K)

    dA1 = np.matmul(dZ2, W2.T)
    dZ1 = np.multiply(dA1, np.multiply(A1, (1 - A1)))
    gradients['W1'] = np.matmul(data.T, dZ1) / m # (input_size, num_hidden)
    gradients['b1'] = np.sum(dZ1, axis = 0) / m # (1, num_hidden)
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    gradients['W1'] += reg * 2 * W1
    gradients['W2'] += reg * 2 * W2
