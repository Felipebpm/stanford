...
def softmax(x):
    ...
    """
    # * START CODE HERE *
    b = np.amax(x)  # for overflow correction
    exp = np.exp(x - b)  # (B, K)
...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # * START CODE HERE *
    params = {}
    params["W1"] = np.random.normal(size=(input_size, num_hidden))
    params["b1"] = np.zeros(num_hidden)
    params["W2"] = np.random.normal(size=(num_hidden, num_output))
...
def forward_prop(data, labels, params):
    ...
    """
    # * START CODE HERE *
    a = sigmoid(np.matmul(data, params["W1"]) + params["b1"])
    o = softmax(np.matmul(a, params["W2"]) + params["b2"])
    ce_loss = -np.sum(np.multiply(labels, np.log(o)), axis=1)
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    grads = {}
    B, K = labels.shape
    activ, y_hat, avg_loss = forward_prop_func(data, labels, params)
    d_z2 = y_hat - labels
    grads["b2"] = (1 / B) * np.sum(d_z2, axis=0)
    grads["W2"] = (1 / B) * np.matmul(activ.T, d_z2)
    d_activ = np.matmul(d_z2, params["W2"].T)
    d_z1 = d_activ * activ * (1 - activ)
    grads["b1"] = (1 / B) * np.sum(d_z1, axis=0)
    grads["W1"] = (1 / B) * np.matmul(data.T, d_z1)
    return grads
    # * END CODE HERE *
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    d_z2 = y_hat - labels
    grads["b2"] = (1 / B) * np.sum(d_z2, axis=0)
    grads["W2"] = (1 / B) * np.matmul(activ.T, d_z2) + reg * 2 * params["W2"]
    d_activ = np.matmul(d_z2, params["W2"].T)
    d_z1 = d_activ * activ * (1 - activ)
    grads["b1"] = (1 / B) * np.sum(d_z1, axis=0)
    grads["W1"] = (1 / B) * np.matmul(data.T, d_z1) + reg * 2 * params["W1"]
    return grads
    # * END CODE HERE *
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for idx in range(0, len(train_data), batch_size):
        ...
        activ, output, avg_loss = forward_prop_func(X, y, params)
